...
void get_keep_alive_time_test() {
    ...
    TEST_ASSERT((time(0) + keep_alive) == get_keep_alive_time(keep_alive)); // this
                                                                            // one
                                                                            // will
                                                                            // pass
}

/* @describe: should pass when entry is updated in table & request respons is a
   success */
void update_service_test() {
    ...
    strncpy(request->service_name, "www.website.com",
            sizeof(request->service_name));
...
/* @describe: should pass when a service is successfully put into the
   lookup_table */
...
/* @describe: should pass when a failure occurs due to the service already being
   defined */
...
/* @describe: should pass when the desired port of a service isn't returned due
   to a timeout */
...
/* @describe: should pass when the a failure occues due to the service not
   existing */
...
void clear_timedout_services_test() {
    ...
    /* initialize request */
    request->msg_type = KEEP_ALIVE;
    strncpy(request->service_name, "www.website0.com",
            sizeof(request->service_name));
    request->port = DESIRED_PORT;
    ...
    strncpy(request->service_name, "www.website1.com",
            sizeof(request->service_name));
    request = define_service(request, lookup_table, keep_alive_time);

    strncpy(request->service_name, "www.website2.com",
            sizeof(request->service_name));
    request = define_service(request, lookup_table, keep_alive_time);

    strncpy(request->service_name, "www.website3.com",
            sizeof(request->service_name));
    request = define_service(request, lookup_table, keep_alive_time);

    strncpy(request->service_name, "www.website4.com",
            sizeof(request->service_name));
    request = define_service(request, lookup_table, keep_alive_time);

    strncpy(request->service_name, "www.website5.com",
            sizeof(request->service_name));
    request = define_service(request, lookup_table, keep_alive_time);

    strncpy(request->service_name, "www.website6.com",
            sizeof(request->service_name));
    request = define_service(request, lookup_table, keep_alive_time);
