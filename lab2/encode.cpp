// ******************************************************
// Declare functions for encoding and decoding requests
//
...
void* encode(request_t *request, void *buff) {
    ...
    strncpy(null_filled_service_name,
            request->service_name,
            MAX_SERVICE_NAME_LEN);

    if (null_pos != 0) {
        memset(null_filled_service_name + null_pos, '\0',
               sizeof(null_filled_service_name) - null_pos);
    }

    memcpy(((request_t *)buff)->service_name, null_filled_service_name,
           sizeof(null_filled_service_name));

    /* check status, return NULL to indicate ERROR */
    if ((((request_t *)buff)->msg_type < DEFINE_PORT) ||
        (((request_t *)buff)->msg_type > STOP)) return NULL;

    /* check message type, return NULL to indicate ERROR */
    if ((((request_t *)buff)->status < SUCCESS) ||
        (((request_t *)buff)->status > UNDEFINED_ERROR)) return NULL;
...
int is_invalid(request_t *request) {
    ...
    if ((request->msg_type < DEFINE_PORT) ||
        (request->msg_type > STOP)) return 1;

    if ((request->status < SUCCESS) ||
        (request->status > UNDEFINED_ERROR)) return 1;

    /* checks if the service_name is null filled */
    char *null_position;
    int   ptr_offset = 0;
    null_position =
        (char *)memchr(request->service_name,
    '\0',
    ...
    if (null_position != NULL) {
        ...
        for (unsigned int i = ptr_offset; i < sizeof(request->service_name);
             i++) {
...
request_t* decode(void *buff, request_t *decoded) {
    ...
    if (buff != decoded) {
        ...
        strncpy(((request_t *)buff)->service_name,
                decoded->service_name,
                MAX_SERVICE_NAME_LEN);
    }
