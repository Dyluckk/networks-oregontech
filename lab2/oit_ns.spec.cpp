...
void define_port_valid() {
    ...
    strncpy(client_buff->service_name,
            test_service_name.c_str(), MAX_SERVICE_NAME_LEN);
    ...
    if (sendto(fd, buffer, sizeof(request_t), 0, (struct sockaddr *)&remaddr,
    ...
    if (recvlen >= 0) {
        ...
        int service_name_cmp_result = strcmp(recieved_buff->service_name,
                                             test_service_name.c_str());
...
void empty_request_bad() {
    ...
    /* create & encode request_t to be sent to the ns */
    void *buffer; //NOTE WILL GENERATE WARNING
                  //BUT THIS IS EXPECTED AND WE WANT IT
