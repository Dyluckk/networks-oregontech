...
void log_incoming_request(request_t *request_buff) {
    ...
    if (request_buff->status ==
        SERVICE_IN_USE) status_string = "SERVICE_IN_USE";

    if (request_buff->status ==
        SERVICE_NOT_FOUND) status_string = "SERVICE_NOT_FOUND";

    if (request_buff->status ==
        ALL_PORTS_BUSY) status_string = "ALL_PORTS_BUSY";
...
request_t* generate_response(request_t       *decoded_request,
                             ns_lookup_table& lookup_table,
                             int              keep_alive_time,
                             unsigned int     max_supported_ports) {
    if (decoded_request->msg_type == DEFINE_PORT) {
        ...
        if (lookup_table.size() < max_supported_ports) {
            decoded_request =
                define_service(decoded_request, lookup_table, keep_alive_time);
        } else {
        ...
        if (lookup_table.size() < max_supported_ports) {
            decoded_request =
                keep_alive(decoded_request, lookup_table, keep_alive_time);
        } else {
            decoded_request = create_ports_full_response(decoded_request);
        }
...
int main(int argc, char **argv) {
    ...
    ns_address.sin_addr.s_addr = htonl(INADDR_ANY); /* set the address for the
                                                       socket */
    ns_address.sin_port        = htons(NS_PORT);    /* set the transport address
                                                       (port #) */
    ...
    while (!turn_off_server) {
        ...
        if (recvlen > 0) {
            ...
            if (response_buff) {
                ...
                /* process */
                freopen(log_file_name.c_str(), "a+", stdout);
                response_buff = generate_response(response_buff,
                                                  lookup_table,
                                                  keep_alive_time,
                                                  max_supported_ports);
                fclose(stdout);
